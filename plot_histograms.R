...
plot_global_histogram <- function(lens_values) {
  fit <- .fit_normal(lens_values)
  mu <- fit$mu
  sigma <- fit$sigma

  x_rng <- range(lens_values, na.rm = TRUE)
  xpad <- diff(x_rng) * 0.1
  x_seq <- seq(x_rng[1] - xpad, x_rng[2] + xpad, length.out = 200)

  df_curve <- data.frame(
    lens = x_seq,
    density = dnorm(x_seq, mean = mu, sd = sigma)
  )
  df_all <- data.frame(lens = lens_values)

  ggplot(df_all, aes(x = lens)) +
    geom_histogram(aes(y = after_stat(density)),
      bins = 30, fill = "#377eb8", color = "black", alpha = 0.6
    ) +
    geom_line(
    ...
    labs(
      title = "Global Lens Histogram with Single‑Normal Fit",
